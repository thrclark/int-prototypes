
<header class="container int-theme-margin p-0 mt-7 mb-3">
    <div class="d-flex align-content-center align-items-center"> <img alt="" src="../img/logo-red.png" class="int-brand-image">
        <div class="d-flex flex-column align-self-center int-lockup-group text-nowrap">
            <div class="int-lockup-org" id="org-name-display">XYZ University</div>
            <h1 class="int-lockup-pagetitle" id="app-name-display">CampusConfirm</h1>
        </div>
        <div class="w-100"></div>
        <div class="d-none d-lg-block text-nowrap">
            <div class="rbt-header-avatar-id">Hello,  username </div>
        </div>
    </div>
</header>
