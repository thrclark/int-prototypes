...
function(a, b) {
    ...
    g.templates = {}, g.regional = {}, g.OPERATORS = {
        ...
        }, g.templates.group = '<dl id="{{= it.group_id }}" class="rules-group-container">   <dt class="rules-group-header">     <div class="btn-group float-right group-actions">   <div class="dropdown"> <button class="btn btn-sm btn-outline-primary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> Add </button> <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">   <button type="button" class="dropdown-item" data-add="rule">         <i class="{{= it.icons.add_rule }}"></i> {{= it.lang.add_rule }}    </button>    <button type="button" class="dropdown-item" data-add="group">           <i class="{{= it.icons.add_group }}"></i> {{= it.lang.add_group }}    </button>   </div> </div>        {{? it.settings.allow_groups===-1 || it.settings.allow_groups>=it.level }}            {{?}}       {{? it.level>1 }}           <button type="button" class="close" data-delete="group"> <span class="rbt-icon-close"></span> </button>      {{?}}     </div>     <div class="btn-group group-conditions">       {{~ it.conditions: condition }}         <label class="btn btn-sm btn-primary">           <input type="radio" name="{{= it.group_id }}_cond" value="{{= condition }}"> {{= it.lang.conditions[condition] || condition }}         </label>       {{~}}     </div>     {{? it.settings.display_errors }}       <div class="error-container"><i class="{{= it.icons.error }}"></i></div>     {{?}}   </dt>   <dd class=rules-group-body>     <ul class=rules-list></ul>   </dd> </dl>', g.templates.rule = '<li id="{{= it.rule_id }}" class="rule-container">   <div class="rule-header">     <div class="btn-group float-right rule-actions"> <button type="button" class="close" data-delete="rule"> <span class="rbt-icon-close"></span> </button>  </div>   </div>   {{? it.settings.display_errors }}     <div class="error-container"><i class="{{= it.icons.error }}"></i></div>   {{?}}   <div class="rule-filter-container"></div>   <div class="rule-operator-container"></div>   <div class="rule-value-container"></div> </li>',
            ...
            if ("function" == typeof c.input) h = c.input.call(this, a, e);
                ...
                case "checkbox": 
                    l.iterateOptions(c.values, function(a, b) {
                        h += "<div class=\"form-check\"" + f + '><input class="form-check-input"  type="' + c.input + '" name="' + e + '" value="' + a + '"><label class="form-check-label">' + b +"</label>"+ "</div> "
                    });
                    break;
                case "select":
    ...
    l.iterateOptions = function(a, b) {
        ...
        "inline" === a.mode ? this.on("afterUpdateRuleFilter", function(b, c) {
            ...
            c.filter && c.filter.description ? (0 === d.length ? (d = $('<button type="button" class="btn btn-sm btn-info filter-description" data-toggle="popover"><i class="' + a.icon + '"></i></button>'), d.prependTo(c.$el.find(h.rule_actions)), d.popover({
                placement: "left",
                container: "body",
                html: !0
            ...
            var d = c.$el.find("button.filter-description");
            c.filter && c.filter.description ? (0 === d.length && (d = $('<button type="button" class="btn btn-sm btn-info filter-description" data-toggle="bootbox"><i class="' + a.icon + '"></i></button>'), d.prependTo(c.$el.find(h.rule_actions)), d.on("click", function() {
        ...
        this.on("afterInit", function() {
            ...
            var e = $(b.value);
            e.find(h.condition_container).after('<button type="button" class="btn btn-sm btn-default" data-invert="group"><i class="' + a.icon + '"></i> ' + c.lang.invert + "</button>"), b.value = e.prop("outerHTML")
        }), a.display_rules_button && a.invert_rules && this.on("getRuleTemplate.filter", function(b) {
            var d = $(b.value);
            d.find(h.rule_actions).prepend('<button type="button" class="btn btn-sm btn-default" data-invert="rule"><i class="' + a.icon + '"></i> ' + c.lang.invert + "</button>"), b.value = d.prop("outerHTML")
        })
    }, {
